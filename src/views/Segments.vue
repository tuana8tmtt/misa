  ...
  <DefaultLayout>
    <div>
      <div class="bread-crumb has-btn">
        <p class="bc-page__title">Manage List Segment</p>
        <v-btn
          depressed
...
import SegmentActions from "@/components/AgTableCustomComponent/CustomCell/SegmentActions"
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          cellRendererFramework: "SegmentActions",
          cellRendererParams: () => {
            return {
  ...
  methods: {
    ...
    async setFav(_val) {
      let res
      if (!_val.favorite) {
        res = await api.segment.setFav(_val.id)
      } else {
        res = await api.segment.unFav(_val.id)
      }
      if (!res) {
        return null
      }
    ...
    async filterChangedCallback(modelObj = null) {
      ...
      this.$router.replace(`/segments/?${filterApplied}`).catch((error) => {
      ...
      if (!this.lockDownFilterApi) {
        ...
        const res = await api.segment.getListData(query)
        if (!res) {
          return null
        }
        try {
          if (res.status > 199 && res.status < 399) {
            const datas = res.data.data.segments.map((o) => {
    ...
    async onGridReady(params) {
      ...
      const res = await api.segment.getListData(
        this.handleRemoveUnusedQuery(queriesComputed),
      )
      if (!res) {
        return null
