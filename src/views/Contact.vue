  ...
  <DefaultLayout>
    <div>
      <div class="bread-crumb has-btn">
        <p class="bc-page__title">All Contacts</p>
        <v-btn
          depressed
...
import EditContact from "@/components/Popup/EditContact.vue"
...
export default {
  ...
  methods: {
    ...
    cellClicked(event) {
      console.log(event)
      if (event.column && event.column.colId !== "action") {
        // window.alert(`Cột ID: {event.value}`);
        const query = { ...this.$route.query }
        this.$router.replace({
          path: `/contacts/${event.data.id}`,
          query: {
            ...query,
    ...
    async filterChangedCallback(modelObj = null) {
      ...
      this.$router.replace(`/contacts/?${filterApplied}`).catch((error) => {
