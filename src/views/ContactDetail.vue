    ...
    <div class="h-100 first-wr-in-df">
      <div class="bread-crumb has-btn">
        <p v-show="!editGeneralInfo" class="bc-page__title">
          ...
          Contact Detail: {{ detail.full_name }}
        </p>
      ...
      <div class="tab-control justify-content-between mb-3">
        <!-- <v-tabs
          v-model="tab"
          class="company-detail-tabs"
          :active-class="'company-detail-tabs-header__active'"
          :hide-slider="true"
        >
      ...
      >
        <v-tab-item>
          <!-- <CompanyContact
            v-if="
              !showDetailContact && !createNewContact && !createFromExistContact
            "
            :current-detail="detail"
            :currentId="id"
            :on-create-new-contact="onCreateNewContact"
            :on-create-from-exist-contact="onCreateFromExistContact"
            :on-show-detail-contact="onShowDetailContact"
          /> -->
          <CompanyContactCreate
            v-if="!showDetailContact && createNewContact"
            :current-detail="detail"
            :currentId="id"
            :updateCurrentData="updateCurrentData"
            :backAction="backContactInfo"
          />
          <CompanyContactCreateFromExist
            v-if="!showDetailContact && createFromExistContact"
            :current-detail="detail"
            :currentId="id"
            :updateCurrentData="updateCurrentData"
            :backAction="backContactInfo"
  ...
  methods: {
    ...
    async getDetailData() {
      ...
      try {
        if (res.status > 199 && res.status < 399) {
          this.detail = res.data.data.contact
    ...
    async onConfirmDelete() {
      ...
      try {
        ...
        const msg =
          "Failed when remove company: " + error.message ||
          error.data.message ||
          String(error) ||
          " Try again"
        this.$store.commit("toast/getError", msg)
