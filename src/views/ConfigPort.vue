  ...
  <DefaultLayout>
    <div>
      <div class="bread-crumb has-btn">
        <p class="bc-page__title">Manage Port</p>
        <v-btn
          depressed
      ...
      <div class="ports-tab h-100 bg-white">
...
import PortEdit from "@/components/Popup/PortEdit.vue"
import PortCreate from "@/components/Popup/PortCreate.vue"
//
...
export default {
  ...
  methods: {
    ...
    async setFav(_val) {
      let res
      if (!_val.favorite) {
        res = await api.port.setFav(_val.id)
      } else {
        res = await api.port.unFav(_val.id)
      }
      if (!res) {
        return null
      }
    ...
    async onConfirmDelete() {
      ...
      const res = await api.port.delete(this.selectedToDelete.id)
      if (!res) {
        return null
      }
      this.loadingDelete = false
    ...
    async onConfirmEdit(data) {
      ...
      const res = await api.port.update(this.selectedToEdit.id, data)
      ...
      try {
        ...
        const msg =
          "Failed when update port: " +
          (res.data.message || String(res) || " Try again")
        this.$store.commit("toast/getError", msg)
      } catch (error) {
        const msg =
          "Failed when update port: " + error.message ||
          error.data.message ||
          String(error) ||
          " Try again"
        this.$store.commit("toast/getError", msg)
    ...
    async filterChangedCallback(modelObj = null) {
      ...
      this.$router.replace(`/config-port?${filterApplied}`).catch((error) => {
      ...
      if (!this.lockDownFilterApi) {
        ...
        const res = await api.port.getListData(query)
        if (!res) {
          return null
        }
        try {
          if (res.status > 199 && res.status < 399) {
            const datas = res.data.data.ports.map((o) => {
    ...
    async onGridReady(params) {
      ...
      const res = await api.port.getListData(
        this.handleRemoveUnusedQuery(queriesComputed),
      )
      if (!res) {
        return null
    ...
    async createData(body) {
      ...
      const res = await api.port.create(body)
      ...
      try {
        if (res.status > 199 && res.status < 399) {
          this.$store.commit("toast/getSuccess", "Create port success")
          this.$store.dispatch("others/getListPort")
          this.filterChangedCallback()
          this.onCancelCreate()
          this.selectedData = {}
        ...
        const msg =
          "Failed when create port: " +
          (res.data.message || String(res) || " Try again")
        this.$store.commit("toast/getError", msg)
      } catch (error) {
        const msg =
          "Failed when create port: " + error.message ||
          error.data.message ||
          String(error) ||
          " Try again"
        this.$store.commit("toast/getError", msg)
