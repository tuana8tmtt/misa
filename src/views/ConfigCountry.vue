  ...
  <DefaultLayout>
    <div>
      <div class="bread-crumb has-btn">
        <p class="bc-page__title">Manage Country</p>
        <!-- <v-btn
          depressed
      ...
      <div class="countries-tab h-100 bg-white">
...
import CountryEdit from "@/components/Popup/CountryEdit.vue"
//
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          // icons: "fa-bars",
          // maxWidth: "auto",
          headerName: "Country Abbreviation",
          filterParams: {
            filterChangedCallback: this.filterChangedCallback,
          },
        },
  ...
  methods: {
    ...
    async setFav(_val) {
      let res
      if (!_val.favorite) {
        res = await api.country.setFav(_val.id)
      } else {
        res = await api.country.unFav(_val.id)
      }
      if (!res) {
        return null
      }
    ...
    async onConfirmDelete() {
      ...
      const res = await api.country.delete(this.selectedToDelete.id)
      if (!res) {
        return null
      }
      this.loadingDelete = false
    ...
    async onConfirmEdit(data) {
      ...
      const res = await api.country.update(this.selectedToEdit.id, data)
    ...
    async filterChangedCallback(modelObj = null) {
      ...
      this.$router
        .replace(`/config-country?${filterApplied}`)
        .catch((error) => {
      ...
      if (!this.lockDownFilterApi) {
        ...
        const res = await api.country.getListData(query)
        if (!res) {
          return null
        }
        try {
          if (res.status > 199 && res.status < 399) {
            const datas = res.data.data.countries.map((o) => {
    ...
    async onGridReady(params) {
      ...
      const res = await api.country.getListData(
        this.handleRemoveUnusedQuery(queriesComputed),
      )
      if (!res) {
        return null
    ...
    async updateData(body) {
      ...
      const res = await api.country.update(this.selectedToEdit.id, body)
      this.loadingEdit = false
      if (!res) {
        return null
      }
      try {
        ...
        const msg =
          "Failed when edit country: " +
          (res.data.message || String(res) || " Try again")
        this.$store.commit("toast/getError", msg)
      } catch (error) {
        const msg =
          "Failed when edit country: " + error.message ||
          error.data.message ||
          String(error) ||
          " Try again"
        this.$store.commit("toast/getError", msg)
