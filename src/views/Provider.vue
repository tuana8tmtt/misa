  ...
  <DefaultLayout>
    <div class="">
      <div class="bread-crumb has-btn">
        <p class="bc-page__title">All Providers</p>
        <v-btn
          depressed
  ...
  methods: {
    ...
    cellClicked(event) {
      if (event.data.id && event.column.colId !== "website") {
        ...
        this.$router.push(`/providers/${event.data.id}`)
      }
    },
    showNoRowsOverlay() {
    ...
    async filterChangedCallback(modelObj = null) {
      ...
      this.$router.replace(`/providers?${filterApplied}`).catch((error) => {
      ...
      if (!this.lockDownFilterApi) {
        ...
        const res = await api.providers.getListData(query)
        if (!res) {
          return null
        }
        try {
