  ...
  <DefaultLayout>
    <div>
      <div class="bread-crumb has-btn">
        <p class="bc-page__title">Manage User</p>
        <v-btn
          depressed
      ...
      <div class="users-tab h-100 bg-white">
      ...
      <UserEdit
        ...
        :on-reset-password="resetPasswordData"
        :on-loading="loadingEdit"
        :on-loading-reset="loadingResetPassword"
        :close-outside="true"
      />
      <UserCreate
...
export default {
  ...
  mixins: [objHandlerMixins, agFilters, dateMixins],
  created() {
  ...
  methods: {
    ...
    async setFav(_val) {
      let res
      if (!_val.favorite) {
        res = await api.users.setFav(_val.id)
      } else {
        res = await api.users.unFav(_val.id)
      }
      if (!res) {
        return null
      }
    ...
    async onConfirmDelete() {
      ...
      const res = await api.users.delete(this.selectedToDelete.id)
      if (!res) {
        return null
      }
      this.loadingDelete = false
    ...
    async onConfirmEdit(data) {
      ...
      const res = await api.users.update(this.selectedToEdit.id, data)
    ...
    async filterChangedCallback(modelObj = null) {
      ...
      let filterApplied = this.parseFilterToParams({
        ...model,
      })
      const localQuery = {
      ...
      this.$router.replace(`/config-user?${filterApplied}`).catch((error) => {
      ...
      if (!this.lockDownFilterApi) {
        ...
        const res = await api.users.getListData(query)
        if (!res) {
          return null
        }
        try {
          if (res.status > 199 && res.status < 399) {
            const datas = res.data.data.users.map((o) => {
    ...
    async onGridReady(params) {
      ...
      const res = await api.users.getListData(
        this.handleRemoveUnusedQuery(queriesComputed),
      )
      if (!res) {
        return null
    ...
    parseDataObj(obj) {
      return {
        ...
        creation_date: this.parseDateString(obj.created_at),
      }
    },
    ...
    async createData(body) {
      ...
      try {
        if (res.status > 199 && res.status < 399) {
          this.$store.commit("toast/getSuccess", "Create user success")
          this.filterChangedCallback()
        ...
        const msg =
          "Failed when create user: " + error.message ||
          error.data.message ||
          String(error) ||
          " Try again"
        this.$store.commit("toast/getError", msg)
    ...
    async updateData(body) {
      ...
      const res = await api.users.update(this.selectedToEdit.id, body)
      this.loadingEdit = false
      if (!res) {
        return null
      }
      try {
        ...
        const msg =
          "Failed when edit user: " +
          (res.data.message || String(res) || " Try again")
        this.$store.commit("toast/getError", msg)
      } catch (error) {
        const msg =
          "Failed when edit user: " + error.message ||
          error.data.message ||
          String(error) ||
          " Try again"
        this.$store.commit("toast/getError", msg)
    ...
    async resetPasswordData() {
      ...
      const res = await api.users.resetPassword(this.selectedToEdit.id)
      this.loadingEdit = false
      ...
      try {
        if (res.status > 199 && res.status < 399) {
          this.$store.commit(
            "toast/getSuccess",
            "Reset user's password success",
          )
          this.filterChangedCallback()
          return
        }
        const msg =
          "Failed when reset user's password: " +
          (res.data.message || String(res) || " Try again")
        this.$store.commit("toast/getError", msg)
      } catch (error) {
        const msg =
          "Failed when reset user's password: " + error.message ||
          error.data.message ||
          String(error) ||
          " Try again"
        this.$store.commit("toast/getError", msg)
