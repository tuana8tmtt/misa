...
export default {
  ...
  watch: {
    ...
    customFilters: {
      deep: true,
      handler() {
        if (this.customFilters && this.customFilters.account_owner_id) {
          this.handlerSyncStoredFilter(this.customFilters.account_owner_id)
        } else {
          this.keySearch = ""
          this.ftValue = null
        }
  ...
  computed: {
    ...
    dataListComputed() {
      const listUserComputed = this.listUser.map((o) => ({
        ...o,
        name_computed: `${o.name}`,
      }))
      return listUserComputed.filter((o) => {
        try {
