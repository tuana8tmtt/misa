...
export default {
  ...
  watch: {
    ...
    customFilters: {
      deep: true,
      handler() {
        if (this.customFilters && this.customFilters.type_id) {
          this.handlerSyncStoredFilter(this.customFilters.type_id)
        } else {
          this.keySearch = ""
          this.ftValue = null
        }
  ...
  computed: {
    ...
    dataListComputed() {
      return this.companyTypes.filter((o) => {
        try {
          const oVal = tStr.removeAscent(o.name).replaceAll(" ", "")
          const kVal = tStr.removeAscent(this.keySearch).replaceAll(" ", "")
