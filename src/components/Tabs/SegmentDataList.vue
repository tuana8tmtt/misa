<template>
  <div class="segment-data-list-tab h-100">
    <div class="each-info-part">
      ...
      >
        <v-btn
          v-if="!isEdit && currentDetail && currentDetail.is_active"
          depressed
          color="#A7A7A700"
          class="
        ...
        <v-btn
          ...
          class="
            ...
            btn-font-weight-regular btn-font-color-66
          "
          :disabled="loadingRemoveContacts"
          @click="isEdit = false"
        >
          <img
      ...
      <v-row v-show="!isNodata" class="each-part pt-1 data-part mt-0">
    ...
    <AddContactToSegment
      :is-visible="onAddContact"
      :on-cancel="onCancelAddContacts"
      :on-confirm="addContactToQuickSegment"
      :on-loading="loadingAddContact"
      :close-outside="true"
    />
  </div>
</template>
...
import AddContactToSegment from "@/components/Popup/AddContactToSegment.vue"
...
export default {
  ...
  data() {
    return {
      ...
      columnDefs: [
        ...
        {
          field: "first_name",
          filterFramework: "agTextColumnFilter",
          // filter: "agSetColumnFilter",
      ...
      lockDownFilterSearch: false, // use to lock api callback when debug
    }
  },
  computed: {
    ...mapState({
  ...
  watch: {
    searchKey() {
      if (this.lockDownFilterSearch) {
        this.lockDownFilterSearch = false
        return
      }
      if (this.refSearch) {
  ...
  methods: {
    ...
    async addContactToQuickSegment(body) {
      this.loadingAddContact = true
      const res = await api.segment.addContact(this.currentId, body)
    ...
    async filterChangedCallback(modelObj = null) {
      ...
      if (this.searchKey) {
        if (filterApplied) {
          filterApplied += `&search=${this.searchKey}`
        } else {
          filterApplied += `search=${this.searchKey}`
        }
      }
      ...
      this.$router
        .replace(`/segments/${this.currentId}/?${filterApplied}`)
        .catch((error) => {
    ...
    clearFilter() {
      this.lockDownFilterSearch = true
      this.searchKey = ""
      this.$router
        .replace({
          query: {
    ...
    async headerCheckBoxClick(event) {
      ...
      const res = await api.contact.getListData(queries)
      if (!res) {
        return null
      }
      try {
        if (res.status > 199 && res.status < 399) {
          const data = res.data.data.contacts.map((o) => o.id)
          if (event.target.checked) {
            this.cacheSelected = [...this.cacheSelected, ...data]
            this.$nextTick(() => {
              this.handleSyncCheckbox()
            })
          } else {
    ...
    async onRemoveContact() {
      const selectedContacs = [...this.cacheSelectedIds]
      const body = {
        contact_ids: selectedContacs.map((o) => o),
      }
      this.loadingRemoveContacts = true
      const res = await api.segment.removeContact(this.currentId, body)
      this.loadingRemoveContacts = false
      if (!res) {
        return null
      }
      try {
        if (res.status > 199 && res.status < 399) {
          this.$store.commit("toast/getSuccess", "Remove contacts success")
          // this.updateCurrentData(res.data.data.company)
          this.filterChangedCallback()
          return
        }
        const msg =
          "Failed when Remove contacts: " +
          (res.data.message || String(res) || " Try again")
        this.$store.commit("toast/getError", msg)
      } catch (error) {
        const msg =
          "Failed when Remove contacts: " + error.message ||
          error.data.message ||
          String(error) ||
          " Try again"
        this.$store.commit("toast/getError", msg)
