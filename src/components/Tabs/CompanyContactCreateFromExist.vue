        ...
        <v-col cols="12" class="data-wr pt-0">
          <div class="filter-block py-3 px-3">
            ...
            <v-btn
              ...
              class="btn-normal-text text-white bc-btn ml-auto"
              :loading="loadingAddContacts"
              @click="onAddContact"
...
export default {
  ...
  methods: {
    ...
    async onAddContact() {
      ...
      if (!selectedContacs || !selectedContacs.length) {
        const msg = "Failed when add contact: " + "0 contact selected"
      ...
      this.loadingAddContacts = true
      const res = await api.company.addContact(this.currentId, body)
      this.loadingAddContacts = false
      if (!res) {
        return null
      }
      try {
        if (res.status > 199 && res.status < 399) {
          this.$store.commit("toast/getSuccess", "Add contacts success")
          this.cacheSelected = []
          this.filterChangedCallback()
          this.backAction()
          return
