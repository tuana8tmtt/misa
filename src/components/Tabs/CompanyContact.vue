<template>
  <div class="company-contact-tab h-100">
        ...
        <v-menu left offset-y>
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              ...
              v-on="on"
            >
              <span class="font-size-20 fw-400 mr-1">+</span> Add Contact
            </v-btn>
          </template>
          <div class="bg-white py-3">
            <p
              class="add-contact-selection pointer"
              @click="onCreateFromExistContact"
            >
        ...
        <v-col cols="12" class="data-wr pt-0">
          <div class="filter-block py-3 px-3">
            <v-menu offset-y :close-on-content-click="false">
              <template v-slot:activator="{ on, attrs }">
                <p class="btn-add-selected pointer" v-bind="attrs" v-on="on">
                  Add selected to segment
                </p>
              </template>
              <div class="bg-white">
                <p
                  class="btn-add-new-segment pointer mb-0"
                ...
                <div class="btn-apply">
                  <v-btn
                    ...
                    class="
                      ...
                      ml-auto
                    "
                    :loading="loadingAddToSegment"
                    @click="addContactToSegment"
                  >
                    Apply
                  </v-btn>
                </div>
...
export default {
  ...
  computed: {
    ...
    cacheSelectedIds() {
      const rss = this.cacheSelected.filter((o) => o && o.id)
      return rss.map((o) => o.id)
    },
    segmentsComputed() {
  ...
  methods: {
    onRowSelected(val) {
      ...
      const data = val.data
      console.log(isSelected, val, "onRowSelected")
      // const idx = this.cacheSelected.findIndex(o => o.id ===data.id )
      if (isSelected) {
        this.cacheSelected.push(data)
      } else {
    ...
    async filterChangedCallback(modelObj = null) {
      ...
      this.$router
        .replace(`/companies/${this.currentId}/?${filterApplied}`)
        .catch((error) => {
    ...
    async addContactToQuickSegment(segmentName) {
      ...
      const rss = this.cacheSelected.filter((o) => o && o.id)
      const selectedContacs = [...new Set(rss.map((o) => o.id))]
      const body = {
        name: segmentName,
    ...
    async addContactToSegment() {
      if (!this.segmentSelected) {
        const msg = "Failed when add contact: " + "segment is required"
        this.$store.commit("toast/getError", msg)
        return
      }
      const rss = this.cacheSelected.filter((o) => o && o.id)
      ...
      this.loadingAddToSegment = true
      const res = await api.segment.addContact(this.segmentSelected, body)
      this.loadingAddToSegment = false
      if (!res) {
        return null
      }
      try {
        if (res.status > 199 && res.status < 399) {
          this.$store.commit(
            "toast/getSuccess",
            "Add contact to segment success",
          )
          // this.updateCurrentData(res.data.data.company)
          this.cacheSelected = []
          this.filterChangedCallback()
        ...
        const msg =
          "Failed when add contact: " +
          (res.data.message || String(res) || " Try again")
        this.$store.commit("toast/getError", msg)
      } catch (error) {
        const msg =
          "Failed when add contact: " + error.message ||
          error.data.message ||
          String(error) ||
          " Try again"
        this.$store.commit("toast/getError", msg)
