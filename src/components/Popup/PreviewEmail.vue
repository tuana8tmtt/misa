    ...
    >
      ...
      <v-card class="popup-preview__card">
        <div class="popup-preview__content">
          <div class="header-preview d-flex align-items-start">
            ...
            <div class="header-preview-tt">
              ...
              <div class="from-to-block">
                <p class="mb-4">From: {{ from }}</p>
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    <p class="mb-0" v-bind="attrs" v-on="on">
                      To:
                      {{
                        content.emails && content.emails.length
                          ? mergeStringFromFlatArray(content.emails.slice(0, 3))
                          : " --- "
                      }}{{
                        content.emails &&
                        content.emails.length &&
                        content.emails.length > 3
                          ? ", and" +
                            ` ${content.emails.length - 3}` +
                            " emails"
                          : ""
                  ...
                  <span>
                    {{
                      content.emails
                        ? mergeStringFromFlatArray(content.emails.slice(3))
                        : ""
                    }}</span
                  >
                </v-tooltip>
          ...
          <p class="mb-3 font-weight-bold">{{ content.subject }}</p>
          <div
